...
def run_pipeline(pdb_id, mol2_path,smiles):
    ...
    pdb_id = str(pdb_id)
    ligand_code= extract_ligand_info(mol2_path)
    smiles = str(smiles)
    ...
    if not ligand_code or not smiles:
        print("Error: Missing required ligand information.")
        sys.exit(1)
    ...
    def run_dpocket(pdb_id, ligand_id,output_directory):
        ...
        pdb_id_with_extension = f'{pdb_id}'
...
if __name__ == "__main__":
    if len(sys.argv) != 4:
        print("Usage: python final_proba_model.py <pdb_id> <mol2_path> <smiles>")
        sys.exit(1)

    pdb_id, mol2_path, smiles = sys.argv[1], sys.argv[2], sys.argv[3]

    run_pipeline(pdb_id, mol2_path, smiles)
