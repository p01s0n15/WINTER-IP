...
def run_pipeline(pdb_id, mol2_path):
    ...
    pdb_id = str(pdb_id)
    ligand_code, smiles = extract_ligand_info(mol2_path)
...
def extract_residue_info(pdb_path):
    parser = PDBParser(QUIET=True)
    structure = parser.get_structure("protein", pdb_path)
    for model in structure:
        for chain in model:
            for residue in chain:
                return residue.get_resname()  # Dynamically extract any residue
    return None
...
def extract_ligand_info(mol2_path):
    ...
    ligand_code = "UNKNOWN_LIGAND"  # ✅ Default ligand name
    smiles = None
    ...
    for i, line in enumerate(lines):
        ...
        if line.startswith("@<TRIPOS>MOLECULE"):
            if i + 1 < len(lines):
                ...
                if potential_name and not potential_name.isnumeric():
                    ligand_code = potential_name  # ✅ Assign actual ligand name

    # ✅ First Attempt: Try RDKit
    try:
        mol = MolFromMol2File(mol2_path, sanitize=True, removeHs=True)
        if mol:
            smiles = MolToSmiles(mol)
    except Exception as e:
        print(f"Warning: RDKit failed to generate SMILES - {e}")

    # ✅ Second Attempt: Try Open Babel
    if not smiles:
        try:
            smiles = subprocess.check_output(["obabel", mol2_path, "-osmi"], text=True).strip()
        except subprocess.CalledProcessError as e:
            print(f"Warning: Open Babel failed to generate SMILES - {e}")

    # ✅ Final Fallback: Assign a Default SMILES
    if not smiles:
        print("⚠️  Warning: Could not extract SMILES. Assigning default 'C'.")
        smiles = "C"  # A single carbon atom as a placeholder

    return ligand_code, smiles
...
if __name__=="__main__":
    ...
    if len(sys.argv) != 3:
        print("Usage: python final_proba_model.py <pdb_id> <mol2_path>")
        sys.exit(1)

    # Assign command-line arguments to variables   UPDAAAAAAAAAAAAAAAAATEEEE
    pdb_id, mol2_path = sys.argv[1], sys.argv[2]
    ligand_code, smiles = extract_ligand_info(mol2_path)
    residue = extract_residue_info(f"{pdb_id}.pdb")


    # Call the run_pipeline function with the obtained values
    run_pipeline(pdb_id, mol2_path)
