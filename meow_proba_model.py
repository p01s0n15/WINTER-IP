...
class PipelineTask:
    ...
    def run_dpocket(self, pdb_id, ligand_code, current_directory, csv_name):
        def generate_input_txt(pdb_id, ligand_id):
            ...
            with open(input_txt, 'w') as file:
                file.write(f'{current_directory}/{pdb_id} {ligand_id}')
            return input_txt

        input_txt = generate_input_txt(f'{pdb_id}.pdb', ligand_code)
    ...
    def run_dssp(self, pdb_file, output_dssp, csv_name):
        ...
        with self.lock:
            try:
                subprocess.run(f"dssp -i {pdb_file} -o {output_dssp}", shell=True)
...
# Continuing from the previous ThreadPoolExecutor block...
